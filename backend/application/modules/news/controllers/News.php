...
function lisiting() {
        ...
        $tables = "events";
        $config['base_url'] = base_url().'news/lisiting/';
       ...
       // $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
		...
		$datahead['lang'] =$lang;
		$data['lang'] =$lang;
		$data['result_amount'] =$this->db->get_where('events',array('view'=>'1'))->result();
        else:
...
$this->load->view('index/include/insidejs');
    ...
    function details() {
      ...
      if($tab_id==""){redirect(DIR."news/lisiting");}
          else {
            global $lang;
            if( isset($this->session->get_userdata('lang')['lang']) ){
                $lang = $this->session->get_userdata('lang')['lang'];
    ...
    $data['site_setting'] =$this->db->get_where('site_settings')->result();
    $data_conent['news_details']=$this->db->get_where('events',array("view"=>'1','id'=>$tab_id))->result();
